import { ParkArea } from "@app/core/models/park-area";
import { mockSpaces } from "./spaces";

export const mockAreas: ParkArea[] = [{
    id: 1,
    parkId: 1,
    name: "basement1",
    description: "description1 description1 description1 description1 description1 ",
    reservationsEnabled: true,
    statusUpdateTime: Date.now(),
    emptySpace: 20,
    reservedSpace: 10,
    occupiedSpace: 5,
    spaces: mockSpaces,
    templateImage: "https://www.realserve.com.au/wp-content/uploads/CarParkingPlans/CAR-PARKING-PLAN-SERVICE-BY.jpg",
    minPrice: 10,
    maxPrice: 20,
    avaragePrice: 15,
    pricings: []
  },
  {
    id: 2,
    parkId: 1,
    name: "basement2",
    reservationsEnabled: true,
    statusUpdateTime: Date.now(),
    emptySpace: 5,
    reservedSpace: 10,
    occupiedSpace: 20,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 20,
    maxPrice: 50,
    avaragePrice: 25,
    pricings: []
  },
  {
    id: 3,
    parkId: 1,
    name: "basement3",
    description: "description3 ",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 5,
    reservedSpace: 5,
    occupiedSpace: 5,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 23,
    maxPrice: 32,
    avaragePrice: 29,
    pricings: []
  },
  {
    id: 4,
    parkId: 2,
    name: "basement4",
    description: "description444444444444444444444444444444444",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 3,
    reservedSpace: 4,
    occupiedSpace: 7,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 24,
    maxPrice: 31,
    avaragePrice: 30,
    pricings: []
  },
  {
    id: 5,
    parkId: 2,
    name: "basement5",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 9,
    reservedSpace: 3,
    occupiedSpace: 5,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 25,
    maxPrice: 30,
    avaragePrice: 28,
    pricings: []
  },
  {
    id: 6,
    parkId: 2,
    name: "basement6",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 290,
    reservedSpace: 310,
    occupiedSpace: 155,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 41,
    maxPrice: 45,
    avaragePrice: 43,
    pricings: []
  },
  {
    id: 7,
    parkId: 2,
    name: "basement7",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 15,
    reservedSpace: 42,
    occupiedSpace: 242,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 33,
    maxPrice: 35,
    avaragePrice: 34,
    pricings: []
  },
  {
    id: 8,
    parkId: 2,
    name: "basement8",
    description: "description8 description8 description8 description8 description8 description8 description8 description8 description8 description8",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 1001,
    reservedSpace: 5,
    occupiedSpace: 5,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 32,
    maxPrice: 44,
    avaragePrice: 43,
    pricings: []
  },
  {
    id: 9,
    parkId: 2,
    name: "basement9",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 141,
    reservedSpace: 124,
    occupiedSpace: 421,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 21,
    maxPrice: 23,
    avaragePrice: 22,
    pricings: []
  },
  {
    id: 10,
    parkId: 3,
    name: "basement10",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 0,
    reservedSpace: 150,
    occupiedSpace: 5,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 22,
    maxPrice: 26,
    avaragePrice: 24,
    pricings: []
  },
  {
    id: 11,
    parkId: 3,
    name: "basement11",
    reservationsEnabled: false,
    statusUpdateTime: Date.now(),
    emptySpace: 1,
    reservedSpace: 1,
    occupiedSpace: 1,
    spaces: mockSpaces,
    templateImage: "https://www.atlanticenter.com/wp-content/uploads/2019/04/floor-plan-first-basement-floor-indoor-parking-spaces-to-rent-milan-1.jpg",
    minPrice: 23,
    maxPrice: 25,
    avaragePrice: 24,
    pricings: [],
  }
];
